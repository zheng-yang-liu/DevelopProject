  ...
  methods: {
    ...
    handleScroll(event) {
      if (event.deltaY > 0) {
        ...
        const articleDetailsOuterBox = document.querySelector(
          '.articleDetailsOuterBox'
        )
        //平滑滚动
        let count = 0
        const intervalId = setInterval(() => {
          //   console.log("我是滑动");
          articleDetailsOuterBox.scrollTop += 6 // 向下滚动33
          count++
          if (count === 6) {
        ...
        const intervalId = setInterval(() => {
          // console.log("我是滑动");
          articleDetailsOuterBox.scrollTop -= 6 // 向上滚动33
          count++
          if (count === 6) {
            // 只执行6次
            clearInterval(intervalId)
